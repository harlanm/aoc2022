...
with open("input.txt") as text_file:
  ...
  text_file.seek(0)
  pairs_two = list(map(json.loads, filter(None, text_file.read().strip().split('\n'))))
...
prev = list()
while True:
  if pairs_two == prev: break
  prev = pairs_two.copy()
  for i in range(len(pairs_two)-1):
    l, r = pairs_two[i], pairs_two[i+1]
    res = compare(iter(pairs_two[i]), iter(pairs_two[i+1]), len(pairs_two[i]), len(pairs_two[i+1]))
    if not res:
      pairs_two[i] = r
      pairs_two[i+1] = l
